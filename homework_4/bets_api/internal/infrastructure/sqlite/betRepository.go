...
func (r *BetRepository) GetBetsByCustomerID(ctx context.Context, id string) ([]domainmodels.Bet, bool, error) {
	storageBets, err := r.queryGetBetsByCustomerID(ctx, id)
	if err == sql.ErrNoRows {
		return []domainmodels.Bet{}, false, nil
	}
	if err != nil {
		return []domainmodels.Bet{}, false, errors.Wrap(err, "bet repository failed to get bets with customer_id " + id)
	}
...
func (r *BetRepository) queryGetBetsByCustomerID(ctx context.Context, id string) ([]storagemodels.Bet, error) {
	row, err := r.dbExecutor.QueryContext(ctx, "SELECT * FROM bets WHERE customer_id='" + id + "';")
	if err != nil {
		return []storagemodels.Bet{}, err
	}
	defer row.Close()
...
func (r *BetRepository) GetBetsByStatus(ctx context.Context, status string) ([]domainmodels.Bet, bool, error) {
	...
	if err != nil {
		return []domainmodels.Bet{}, false, errors.Wrap(err, "bet repository failed to get bets with status " + status)
	}
	var domainBets []domainmodels.Bet
